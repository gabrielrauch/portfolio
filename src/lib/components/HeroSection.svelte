	...
	function typeNextCharacter() {
		...
		if (currentCharIndex < currentLine.text.length) {
			...
			visibleLines = visibleLines.map((line, index) =>
				index === visibleLines.length - 1 ? { ...line, displayText: newDisplayText } : line
			);
			currentCharIndex++;
...
>
	<div class="container mx-auto">
		<div class="mx-auto max-w-4xl space-y-8 text-center">
			...
			<div class="terminal-container glass-card mx-auto w-full max-w-3xl rounded-lg p-6 text-left">
				<div
				...
				<div class="terminal-content font-mono text-sm" style="min-height: 200px;">
					{#each visibleLines as line, i}
						<div class="terminal-line mb-1">
							{#if line.type === 'command'}
								<div class="flex items-center gap-2">
									<span class="text-green-400">$</span>
									<span class="command-text">{line.displayText.replace('$ ', '')}</span>
							...
							{:else if line.text === description}
								<div class="text-muted-foreground pl-4 text-xs leading-relaxed">
									{line.displayText}
									{#if i === visibleLines.length - 1 && isTyping}
										<span class="typing-cursor inline-block h-3 w-1 bg-blue-400"></span>
									{/if}
								</div>
							{:else}
								<div class="output-text pl-4">
									{line.displayText}
									{#if i === visibleLines.length - 1 && isTyping}
										<span class="typing-cursor inline-block h-4 w-1 bg-blue-400"></span>
									{/if}
								</div>
							{/if}
						</div>
			...
			<!-- Status indicator -->
			<div class="text-muted-foreground flex items-center justify-center gap-2 pt-8 text-sm">
